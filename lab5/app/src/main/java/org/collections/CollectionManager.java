...
public class CollectionManager implements Saveable, Loadable {
    ...
    /**
     * Loads collection from file with modification check.
     * @throws FileModifiedException if file was modified since last save
     */
    public void load() throws FileModifiedException {
        collection.clear();
        try {
            SaveData saveData = jsonHandler.read();
            setCurrentId(saveData.getCurrentId());
            
            if (saveData.getLastSaveTime().equals(jsonHandler.getModifDateTime())){
